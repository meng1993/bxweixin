...
Page({
  ...
  searchCondition() {//获取搜索条件列表
    app.request('book/index/searchCondition', 'GET', { 'type': '2' }, 'searchConditionb', this);
  },
...
//    console.log(data.data)
  ...
  benshu(e){
    let index = e.currentTarget.id;
    let param = new Object();
    param['lists4.data[' + this.data.ben + '].checked'] = false;
    this.setData(param);//取选旧的
    if (index == this.data.ben && this.data.benhave == true) {
      this.setData({ benhave: false })
      return false;
    }
    let param2 = new Object();
    param2['lists4.data[' + index + '].checked'] = true;
    this.setData(param2);//选新的
    this.setData({ ben: index, benhave: true })
  },
  go(e) {
    ...
    let common_params = [];
    common_params[0] = {}; common_params[1] = {}; common_params[2] = {}; common_params[3] = {};
    common_params[0].checked_ids = []; common_params[1].checked_ids = []; common_params[2].checked_ids = []; common_params[3].checked_ids = [];
    ...
    common_params[3].condition_id = 2;//本数
    if (this.data.benhave == true) {
      common_params[3].checked_ids[0] = this.data.lists4.data[this.data.ben].param_id;
      searchbiaoqian.unshift(this.data.lists4.data[this.data.ben].name);
    } else {
      common_params[3].checked_ids = [];
    }
    ...
    wx.setStorageSync('search2', data);
    wx.setStorageSync('searchbiaoqian2', searchbiaoqian);
    let _from = ''
    if (this.data.zhineng == true) {
      _from = '智能'
    }
    wx.navigateTo({
      url: '/pages/index/search2/search2?'
    })
  }
})