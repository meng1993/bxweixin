...
Page({
  onLoad(e){ 
    ...
    if(e.from==="list"){//从公共列表进入，未创建单
      ...
      this.setData({ user_bargain_id: e.bargain_id || e.user_bargain_id});
      this.detail2();
    }
  ...
  onShareAppMessage(res){
    if (res.from === 'menu'){
      this.setData({ otherid:0});
    }
    let id = this.data.bargain_id;
    let url = "/pages/kanjia/detail/detail?from=list&bargain_id="
    if (this.data.otherid != 0) {//其它单分享
      id = this.data.otherid;
      url = "/pages/kanjia/info/info?from=share&user_bargain_id="
    }
    return {
  ...
  detailb(data){ 
    ...
    this.setData({
      ...
      card: data.data.card,
      valid_time: Math.floor(data.data.valid_time / 3600)
    });
  ...
  getlist() {//我的当前类其它砍价单
    ...
    app.request('book/bargain/detail_for_owner', 'POST', { user_id: user_id, token: token, user_bargain_id:id,
      bargain_id: bargain_id}, 'getlistb', this)
  },
  getlistb(data) {
  ...
  kanjiab(data){//发起新单回调
    if(data.code==400){
      ...
      setTimeout(() => {
        wx.redirectTo({ url: '/pages/kanjia/info/info?user_bargain_id=' + user_bargain_id + '&from=my' })
      }, 800)
    } else{
      app.tishi('提示',data.msg);
    }
