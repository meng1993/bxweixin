...
Page({
  ...
  detail() {
    ...
    let json = { token: token, user_id: user_id, user_card_id: order_id }
    app.request('book/card/card_order_info', 'POST', json, 'detailb', this);
  },
  detailb(data) {
    console.log('11111111111------------>'+JSON.stringify(data))
    let _data = data.data.card_info;
    let coupon = data.data.coupon_info;
    _data.create_time = app.shijianchuo3(_data.create_time);
     this.setData({ data: _data, coupon: coupon})
  },
  gozhanshi(e) {//展示列表
